...
def analyze_sensor_data(df: pd.DataFrame, sensor_cols: list) -> dict:
    ...
    return {'n_samples': len(df), 'n_sensors': len(sensor_cols), 'mean_values': df[sensor_cols].mean().to_dict(), 'std_values': df[sensor_cols].std().to_dict(), 'missing_values': df[sensor_cols].isnull().sum().to_dict()}

def plot_sensor_data(df: pd.DataFrame, sensor_cols: list, title: str, output_path: Path, plot: bool=False):
    """Plot sensor data"""
    if not plot:
        return
